...
options {
	...
	dump-file                         # DNS cahce dump 파일 생성 디렉토리 경로
	...
	session-keyfile "/run/named/session.key";     # TSIG session key (Master ↔ Salve 보안 트랙잭션 설정)
};
  ```
...
- 도메인명은 항상 마침표(.) 또는 ORIGIN(@)으로 끝나야 합니다. 만약 마침표(.) 또는 ORIGIN(@)이 없을 경우 /var/named/rfc~ 에서 설정했던 도메인명을 불러ㅇ
