...
## 홈페이지 속도 최적화 (Apache + Tomcat)

### Apache 설정 (conf.module.d/00-base.conf)
...
**만료 헤더(mod\_expires)** — 정적 리소스는 1개월, HTML은 즉시 만료로 설정
...
<!-- 이 외 폰트/이미지/오디오/비디오 타입도 동일 패턴으로 1개월 지정 -->
<IfModule mod_headers.c>
...
### Tomcat 설정 (conf/server.xml) — AJP 커넥터 압축
